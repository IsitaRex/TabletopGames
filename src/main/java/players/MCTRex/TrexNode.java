...
class TrexNode {
    ...
    Map<AbstractAction, TrexNode> children = new HashMap<>();
    // Depth of this node
    final int depth;
    ...
    protected TrexNode(MCTRexPlayer player, TrexNode parent, AbstractGameState state, Random rnd) {
        this.player = player;
        this.fmCallsCount = 0;
        this.parent = parent;
    ...
    void mctsSearch() {
        ...
        MCTRexParams params = player.getParameters();
    ...
    private TrexNode expand() {
        ...
        TrexNode tn = new TrexNode(player, this, nextState, rnd);
    ...
    private AbstractAction ucb() {
        ...
        for (AbstractAction action : children.keySet()) {
            TrexNode child = children.get(action);
            if (child == null)
            ...
            if (params.progressiveBias) {
                // If we are using progressive bias, we add a term to the UCT value
                double bias = params.progressiveBiasHeuristic.evaluateState(child.state, player.getPlayerID())/ (child.nVisits + params.epsilon);
//                double bias = params.progressiveBiasHeuristic.evaluateState(child.state, this.state.getCurrentPlayer())/ (child.nVisits + params.epsilon);
                uctValue += bias;
            }
    ...
    AbstractAction bestAction() {
        ...
        for (AbstractAction action : children.keySet()) {
            if (children.get(action) != null) {
                TrexNode node = children.get(action);
